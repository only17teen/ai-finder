    ...
    async def get(self) -> T:
        if self._i is None:
            async with self._lk():
                if self._i is None:
                    r = self._f()
                    self._i = await r if inspect.isawaitable(r) else r
        return self._i  # type: ignore[return-value]

    def override(self, v: T) -> None:
        self._i = v
...
class Container:
    ...
    async def resolve(self, t: type[T]) -> T:
        if t not in self._p:
            raise KeyError(f"No provider for {t!r}")
