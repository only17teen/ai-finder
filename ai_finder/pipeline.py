...
def source_registry(db: DB, cfg: dict) -> dict:
    ...
    return {
        "hackernews": lambda: hackernews.collect(db, cfg.get("limits", {}).get("hackernews", 100)),
        "linux_forums": lambda: linux_forums.collect(db),
        "apify": lambda: apify_sources.collect(db, cfg.get("apify", {}).get("token", "")),
...
async def collect(db: DB, cfg: dict, only: str | None) -> int:
    ...
    results = await asyncio.gather(*[registry[n]() for n in names], return_exceptions=True)
    total = 0
...
async def verify_pending(
    db: DB, concurrency: int = 6, retry_cooldown_h: float = 24.0, max_verify: int = 100
) -> int:
