...
async def build_container()->"Container":
    ...
    async def _reg()->PluginRegistry:
        lim=await c.resolve(RateLimiter); r=PluginRegistry(); r.discover(); r.configure_from_meta(lim); return r
    c.singleton(PluginRegistry,_reg)
    return c
def build_command_bus()->"CommandBus":
    from ai_finder.commands import create_default_command_bus; return create_default_command_bus()
def build_query_bus()->"QueryBus":
    from ai_finder.queries import QueryBus; return QueryBus()
