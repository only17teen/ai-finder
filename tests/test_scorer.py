...
def test_niche_bonus_applies_to_under_the_radar():
    niche = {"has_api": 1, "has_referral": 0, "platforms": "reddit", "upvotes": 5}
...
def test_rescore_all_ranks(tmp_path):
    ...
    hi, _ = db.upsert_candidate(
        Candidate(
            url="https://hi.ai",
            name="CodeBot",
            description="AI coding agent API",
            source_platform="hn",
        )
    )
    db.update_service(hi, has_api=1, has_referral=1, referral_commission="40%", upvotes=300)
    lo, _ = db.upsert_candidate(
        Candidate(url="https://lo.ai", name="Mystery", description="some app", source_platform="hn")
    )
    n = rescore_all(db)
    assert n == 2
    top = db.top(1)[0]
