...
def test_card_skips_non_ai():
    assert card_to_candidate({"url": "https://recipes.example", "title": "my cooking blog"}) is None


def test_card_skips_mastodon_and_shorteners():
    assert card_to_candidate({"url": "https://mastodon.uno/x", "title": "AI news"}) is None
    assert card_to_candidate({"url": "https://bit.ly/abc", "title": "great AI tool"}) is None
